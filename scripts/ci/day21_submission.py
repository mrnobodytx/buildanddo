...
CHALLENGE_START = datetime(2026, 9, 3, tzinfo=timezone.utc)
CHALLENGE_DEADLINE = datetime(2026, 9, 25, tzinfo=timezone.utc)
...
def strict_json(path: Path, max_bytes: int = 4_000_000) -> dict[str, Any]:
    ...
    try:
        value = json.loads(raw, object_pairs_hook=pairs, parse_constant=lambda _: (_ for _ in ()).throw(Day21Error("non-finite JSON")))
...
def validate_ref(root: Path, ref: object, now: datetime) -> dict[str, str]:
    ...
    return {"path": str(ref["path"]), "sha256": actual, "observed_at": observed.isoformat()}


def require_challenge_window(refs: Iterable[dict[str, str]], label: str) -> None:
    for ref in refs:
        observed = utc(ref["observed_at"])
        if not CHALLENGE_START <= observed < CHALLENGE_DEADLINE:
            raise Day21Error(f"{label} evidence falls outside the 21-day build window: {ref['path']}")
...
def validate_acceptance(evidence: Path, now: datetime) -> dict[str, Any]:
    ...
    if observed > now:
        raise Day21Error("acceptance summary is observed in the future")
    if data.get("state") != "PASS":
        raise Day21Error("all 18 required acceptance profiles must PASS")
    if data.get("expected_profiles") != 18:
        raise Day21Error("acceptance summary must cover exactly 18 required profiles")
    profiles = data.get("profiles")
    if not isinstance(profiles, dict) or len(profiles) != 18 or any(value != "PASS" for value in profiles.values()):
    ...
    if not isinstance(raw_evidence, dict) or set(raw_evidence) != set(profiles):
        raise Day21Error("acceptance summary must bind receipt/log evidence for every profile")
    verified_evidence = {}
    for key, refs in raw_evidence.items():
        ...
        require_challenge_window(pair.values(), f"acceptance {key}")
        verified_evidence[key] = pair
    candidate = _require_text(data.get("candidate_sha"), "candidate_sha")
    if not re.fullmatch(r"[0-9a-f]{7,64}", candidate):
        raise Day21Error("acceptance candidate_sha must be a git SHA")
    source = _require_text(data.get("source_sha256"), "source_sha256")
    if not re.fullmatch(r"[0-9a-f]{64}", source):
        raise Day21Error("acceptance source_sha256 must be a SHA-256")
    return {"state": "PASS", "candidate_sha": candidate, "source_sha256": source, "observed_at": observed.isoformat(), "profiles": profiles, "evidence": verified_evidence}
...
def validate_public_url(evidence: Path, now: datetime) -> dict[str, Any]:
    ...
    observed = utc(data.get("observed_at"))
    if observed > now:
        raise Day21Error("public URL observation is in the future")
    ...
    return {"url": url, "status_code": status, "observed_at": observed.isoformat(), "body_sha256": data["body_sha256"]}
...
def validate_products(evidence: Path, now: datetime) -> dict[str, Any]:
    ...
    for value in raw:
        ...
        require_challenge_window(verified, product_id)
        by_id[product_id] = {"id": product_id, "name": REQUIRED_PRODUCTS[product_id], "meaningful_use": use, "evidence": verified}
    missing = sorted(set(REQUIRED_PRODUCTS) - set(by_id))
    if missing:
        raise Day21Error("missing Hostinger product proof: " + ", ".join(missing))
    return {"products": [by_id[key] for key in REQUIRED_PRODUCTS], "complete": True}
...
def validate_browser(evidence: Path, now: datetime) -> dict[str, Any]:
    ...
    candidate = _require_text(data.get("candidate_sha"), "candidate_sha")
    if not re.fullmatch(r"[0-9a-f]{7,64}", candidate):
        raise Day21Error("candidate_sha must be a git SHA")
    ...
    if not isinstance(steps, list) or len(steps) < 5:
        raise Day21Error("browser journey must record at least five ordered steps")
    required_kinds = {"public_entry", "auth", "challenge", "mission", "evidence", "operator_readback"}
    kinds = set()
    ...
    for index, step in enumerate(steps):
        ...
        if kind in required_kinds and not verified_refs:
            raise Day21Error(f"required browser step has no screenshot evidence: {kind}")
        screenshots.extend(verified_refs)
    missing = sorted(required_kinds - kinds)
    if missing:
        raise Day21Error("browser journey missing required step kinds: " + ", ".join(missing))
    ...
    require_challenge_window(screenshots, "browser")
    ...
    return {"candidate_sha": candidate, "started_at": started.isoformat(), "finished_at": finished.isoformat(), "step_count": len(steps), "screenshots": screenshots, "console_error_count": console_errors}


def validate_demo_replay(evidence: Path) -> dict[str, Any]:
    ...
    for field in ("candidate_sha", "capture_sha256", "workspace", "mission", "dispatch", "producer", "verifier"):
        _require_text(data.get(field), field)
    if data["producer"] == data["verifier"]:
        raise Day21Error("producer and verifier must be distinct")
    return {key: data[key] for key in ("status", "candidate_sha", "capture_sha256", "workspace", "mission", "dispatch", "producer", "verifier")}


def validate_simple_artifact(evidence: Path, filename: str, schema: str, now: datetime) -> dict[str, Any]:
    ...
    verified = [validate_ref(evidence, ref, now) for ref in refs]
    require_challenge_window(verified, filename)
...
def repository_audit(root: Path) -> dict[str, Any]:
    ...
    gaps = []
    if missing:
        gaps.append({"id": "canonical_source", "state": "BLOCKED", "detail": "missing: " + ", ".join(missing)})
    if not page.is_file():
        gaps.append({"id": "judge_page", "state": "MISSING", "detail": "public Hostinger challenge page absent"})
    if not route.is_file() or "HostingerChallengePage" not in route.read_text(encoding="utf-8", errors="replace"):
        gaps.append({"id": "judge_route", "state": "MISSING", "detail": "public challenge route absent"})
    if not public_pages.is_file() or "/hostinger-challenge" not in public_pages.read_text(encoding="utf-8", errors="replace"):
        gaps.append({"id": "judge_discovery", "state": "MISSING", "detail": "challenge page absent from public page catalog"})
    if not gitlab_fragment.is_file():
        gaps.append({"id": "self_hosted_acceptance", "state": "MISSING", "detail": "GitLab day-21 acceptance lane absent"})
    origins: dict[str, str] = {}
    if public_pages.is_file():
        match = re.search(r"SITE_ORIGIN\s*=\s*['\"]([^'\"]+)", public_pages.read_text(encoding="utf-8", errors="replace"))
        if match:
            origins["site_origin"] = match.group(1).rstrip("/")
    readme = root / "README.md"
    if readme.is_file():
        match = re.search(r"\*\*Live site:\*\*\s*(https://\S+)", readme.read_text(encoding="utf-8", errors="replace"))
        if match:
            origins["readme_live_site"] = match.group(1).rstrip("/")
    if len(set(origins.values())) > 1:
        gaps.append({"id": "canonical_domain", "state": "HOLD", "detail": f"public origin mismatch: {origins}"})
    return {"schema": "buildanddo.day21-repo-audit/v1", "missing_required_paths": missing, "origins": origins, "gaps": gaps, "state": "PASS" if not gaps else "HOLD"}


def evidence_audit(evidence: Path, now: datetime | None = None) -> dict[str, Any]:
    now = now or datetime.now(timezone.utc)
    states: dict[str, dict[str, Any]] = {}
    validators = {
        "acceptance-summary.json": lambda: validate_acceptance(evidence, now),
        ...
        "demo-replay.json": lambda: validate_demo_replay(evidence),
        "architecture.json": lambda: validate_simple_artifact(evidence, "architecture.json", "buildanddo.day21-architecture/v1", now),
        "build-journey.json": lambda: validate_simple_artifact(evidence, "build-journey.json", "buildanddo.day21-build-journey/v1", now),
    }
    for name in REQUIRED_EVIDENCE:
        if not (evidence / name).is_file():
            states[name] = {"state": "MISSING", "reason": "required evidence file absent"}
            continue
        try:
            states[name] = {"state": "PASS", "value": validators[name]()}
        except (OSError, Day21Error) as exc:
    ...
    for name in OPTIONAL_EVIDENCE:
        if not (evidence / name).is_file():
            ...
            continue
        try:
            states[name] = {"state": "PASS", "value": validate_simple_artifact(evidence, name, optional_schemas[name], now)}
        except (OSError, Day21Error) as exc:
            states[name] = {"state": "HOLD", "reason": str(exc)}
    complete = all(states[name]["state"] == "PASS" for name in REQUIRED_EVIDENCE) and not any(states[name]["state"] == "HOLD" for name in OPTIONAL_EVIDENCE)
    return {"schema": "buildanddo.day21-evidence-audit/v1", "state": "PASS" if complete else "HOLD", "items": states}


def rubric_coverage(audit: dict[str, Any]) -> dict[str, Any]:
    passed = {name for name, value in audit["items"].items() if value.get("state") == "PASS"}
    mapping = {
        "product_user_experience": {"acceptance-summary.json", "public-url.json", "browser-journey.json", "demo-replay.json"},
        "business_idea": {"public-url.json", "build-journey.json"},
        "business_potential": {"public-url.json", "architecture.json", "build-journey.json"},
        "hostinger_product_usage": {"hostinger-products.json"},
        "creativity_innovation": {"acceptance-summary.json", "architecture.json", "demo-replay.json", "build-journey.json"},
    }
    rows = {}
    weighted = 0.0
    for key, needed in mapping.items():
        ...
        rows[key] = {"weight": RUBRIC[key], "evidence_coverage": round(fraction, 3), "required_artifacts": sorted(needed), "missing": sorted(needed - passed)}
    return {"note": "Evidence coverage is not a predicted judge score.", "weighted_evidence_coverage": round(weighted, 2), "areas": rows}


def _copy_ref_files(evidence: Path, bundle: Path, audit: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for value in audit["items"].values():
        ...
        while stack:
            ...
            if isinstance(item, dict):
                if set(item) >= {"path", "sha256", "observed_at"} and isinstance(item.get("path"), str):
                    refs.add(item["path"])
    ...
    for name in (*REQUIRED_EVIDENCE, *OPTIONAL_EVIDENCE):
        ...
        shutil.copy2(source, target)
        copied.append({"path": "evidence/manifests/" + name, "sha256": file_digest(target)})
    return copied


def compile_bundle(root: Path, evidence: Path, output: Path, *, title: str, target_audience: str, problem: str, solution: str, pitch: str) -> dict[str, Any]:
    repo = repository_audit(root)
    ev = evidence_audit(evidence)
    if repo["state"] != "PASS":
        raise Day21Error("repository submission audit is not PASS; resolve source/domain gaps first")
    if ev["state"] != "PASS":
        raise Day21Error("required submission evidence is incomplete; run audit for details")
    ...
    if replay["candidate_sha"] != browser["candidate_sha"] or browser["candidate_sha"] != acceptance["candidate_sha"]:
        raise Day21Error("acceptance, browser journey and demo replay must describe the same candidate SHA")
    ...
    if canonical_origin:
        ...
        if public_origin != canonical_origin:
            raise Day21Error("public URL evidence does not match the repository canonical origin")
...
"""
    ...
    optional_rows = [f"- {name}: {ev['items'][name]['state']}" for name in OPTIONAL_EVIDENCE]
    checklist = """# Final owner submission checklist
...
""" + "\n".join(optional_rows) + "\n"
    ...
    index = {"schema": "buildanddo.day21-bundle-index/v1", "files": []}
    for path in sorted(p for p in output.rglob("*") if p.is_file()):
        rel = path.relative_to(output).as_posix()
        if rel == "BUNDLE_INDEX.json":
            continue
        index["files"].append({"path": rel, "sha256": file_digest(path), "bytes": path.stat().st_size})
    index["root_digest"] = digest(index["files"])
    (output / "BUNDLE_INDEX.json").write_text(json.dumps(index, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return manifest
...
def write_templates(evidence: Path) -> None:
    ...
    templates = {
        "acceptance-summary.json": {"schema": "buildanddo.day21-acceptance-summary/v1", "observed_at": "REPLACE", "state": "UNMEASURED", "candidate_sha": "REPLACE", "source_sha256": "REPLACE", "expected_profiles": 18, "profiles": {}, "evidence": {}},
        "public-url.json": {"schema": "buildanddo.day21-public-url/v1", "url": "https://buildanddo.example", "status_code": 0, "observed_at": "REPLACE", "body_sha256": "REPLACE"},
        "hostinger-products.json": {"schema": "buildanddo.day21-hostinger-products/v1", "products": [{"id": key, "meaningful_use": "REPLACE WITH OBSERVED USE", "evidence": []} for key in REQUIRED_PRODUCTS]},
        "browser-journey.json": {"schema": "buildanddo.day21-browser-journey/v1", "candidate_sha": "REPLACE", "started_at": "REPLACE", "finished_at": "REPLACE", "console_error_count": 0, "steps": []},
        "demo-replay.json": {"status": "UNMEASURED", "synthetic": True},
        "architecture.json": {"schema": "buildanddo.day21-architecture/v1", "title": "BuildAndDo architecture", "evidence": []},
        "build-journey.json": {"schema": "buildanddo.day21-build-journey/v1", "title": "21-day build journey", "evidence": []},
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("command", choices=["audit", "templates", "bundle"])
    ...
    parser.add_argument("--target-audience", default="Small business owners and operators")
    parser.add_argument("--problem", default="Operational work is scattered across tools, making it hard to know what changed and what to do next.")
    parser.add_argument("--solution", default="BuildAndDo turns an observed business problem into a bounded mission, verifies the result, and keeps the evidence.")
    parser.add_argument("--pitch", default="BuildAndDo watches the important parts of a small business, explains what changed, and turns the next best action into a verified task.")
    ...
    try:
        ...
        if args.command == "audit":
            result = {"repository": repository_audit(root), "evidence": evidence_audit(evidence) if evidence.exists() else {"state": "MISSING", "items": {}}, "official_required_products": REQUIRED_PRODUCTS, "rubric": RUBRIC}
            result["state"] = "PASS" if result["repository"]["state"] == "PASS" and result["evidence"]["state"] == "PASS" else "HOLD"
            print(json.dumps(result, indent=2, sort_keys=True) if args.json else f"DAY21 {result['state']}\nrepo={result['repository']['state']} evidence={result['evidence']['state']}")
            return 0 if result["state"] == "PASS" else 1
        manifest = compile_bundle(root, evidence, output, title=args.title, target_audience=args.target_audience, problem=args.problem, solution=args.solution, pitch=args.pitch)
        print(json.dumps(manifest, indent=2, sort_keys=True) if args.json else f"READY_FOR_OWNER_REVIEW: {output}")
        return 0
    except (OSError, Day21Error, KeyError) as exc:
