...
| `builder-mailer.pb.js` | Intercepts outgoing account mail. With `smtp.enabled` in PocketBase settings, PocketBase's own SMTP sends it. Otherwise, with `BUILDER_MAILER_PROVIDER=customerio`, `customerio-mail.js` hands the rendered message to Customer.io's transactional API: `CUSTOMERIO_APP_API_KEY`, `CUSTOMERIO_REGION` (`us` or `eu`, default `us`), `BUILDER_MAILER_SENDER_ADDRESS` (must be a verified Customer.io sender) and optional `BUILDER_MAILER_SENDER_NAME` / `BUILDER_MAILER_REPLY_TO`. Open/click tracking and body retention are turned off per message, so a reset link is never rewritten through a tracking host or kept in delivery history. Customer.io creates a person for each new recipient address. Otherwise the message is posted to the builder mailer API (`BUILDER_MAILER_API_*`). | With SMTP disabled and no provider configured, anything that sends mail (password reset, verification) fails with a 500 and is logged; a password-reset request still answers 204. Nothing in a normal local flow sends mail; if you need it, enable SMTP in the admin UI. |
