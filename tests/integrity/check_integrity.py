# ─── CGRF Header ──────────────────────────────
# File:        tests/integrity/check_integrity.py
# Stage:       08_TEST
...
# Created:     2026-09-23
# Depends:     tests.integrity.test_integrity.py, apps/integrity
# EnumType:    Test
# EnumEdges:   DEPENDS_ON tests.integrity.test_integrity.py; VALIDATES apps/integrity
# DAG Node:    none
# Intent:      Require the integrity suite and at least 80 percent statement coverage per module.
# ───────────────────────────────────────────────────────────

"""Run integrity behavior tests and measure dependency-free statement coverage."""
...
def main() -> int:
    """Fail when behavior tests fail or a integrity module measures below 80 percent."""
    sys.path.insert(0, str(ROOT))
    tracer = trace.Trace(count=True, trace=False, ignoredirs=[sys.base_prefix])

    def run() -> unittest.TestResult:
        suite = unittest.defaultTestLoader.loadTestsFromNames(["tests.integrity.test_integrity"])
        return unittest.TextTestRunner(verbosity=1).run(suite)

    result = tracer.runfunc(run)
    counts = tracer.results().counts
    paths = sorted(
        path for path in (ROOT / "apps/integrity").glob("*.py")
    ...
    destination = ROOT / "reports/coverage/integrity.json"
