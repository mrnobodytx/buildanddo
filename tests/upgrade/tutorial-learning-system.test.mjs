...
// Created:     2026-09-19
// Depends:     tests/upgrade/tutorial-learning-fixture.mjs, apps/pocketbase/pb_hooks/tutorial-learning.pb.js, apps/pocketbase/pb_migrations/1790600000_tutorial_learning.js, apps/pocketbase/pb_migrations/1791500000_learning_progress_authority.js, apps/pocketbase/pb_migrations/1791500100_tutorial_answer_wait.js
// EnumType:    Test
// EnumEdges:   CONSUMES tests/upgrade/tutorial-learning-fixture.mjs; VALIDATES apps/pocketbase/pb_hooks/tutorial-learning.pb.js; VALIDATES apps/pocketbase/pb_migrations/1790600000_tutorial_learning.js; VALIDATES apps/pocketbase/pb_migrations/1791500000_learning_progress_authority.js; VALIDATES apps/pocketbase/pb_migrations/1791500100_tutorial_answer_wait.js
// DAG Node:    none
...
import { learningFixture, MIGRATION, PROGRESS_MIGRATION, WAIT_MIGRATION } from './tutorial-learning-fixture.mjs';
...
test('learner lessons stay keyless after completion and only earned feedback explains the answer', () => {
    ...
    assert.ok(hidden(passed), 'completed receipts still use the keyless learner contract');
    ...
    assert.equal(f.detail().tutorial.lesson.check.explanation, undefined);
...
test('answer-wait migration normalizes native field types without accepting schema drift', () => {
    for (const expectedType of ['date', 'text']) for (const representation of ['method', 'boxed-method', 'boxed']) {
        const f = learningFixture(); f.finish();
        const field = f.collections.tutorial_learning.fields.getByName('answer_retry_at');
        field.type = representation === 'boxed' ? new String(expectedType) : function () {
            assert.equal(this, field, 'native field methods keep their receiver');
            return representation === 'boxed-method' ? new String(expectedType) : expectedType;
        };
        const type = field.type, before = plain(f.data);
        let saves = 0; const save = f.app.save;
        f.app.save = (value) => { saves++; return save.call(f.app, value); };
        const migration = f.migration(WAIT_MIGRATION);
        if (expectedType === 'date') {
            migration.up(); migration.up();
            assert.equal(f.collections.tutorial_learning.fields.getByName('answer_retry_at').type, type);
            assert.equal(f.list().points, 100);
        } else {
            assert.throws(() => migration.up(), /Review custom tutorial_learning\.answer_retry_at/);
        }
        assert.equal(saves, 0, `${representation}: replay and drift rejection do not write`);
        assert.equal(f.collections.tutorial_learning.fields.filter((item) => item.name === 'answer_retry_at').length, 1);
        assert.deepEqual(f.data, before, 'snapshots, checkpoints, certificates and historical records stay unchanged');
    }
});

test('progress authority and answer waits install together and replay without changing snapshots or historical reading', () => {
    const f = learningFixture({ progressAuthority: false });
    f.migration(WAIT_MIGRATION).down();
    assert.throws(() => f.command('start'), /upgrade is not installed/);
    f.seed('tutorial_progress', { owner: 'owner', tutorial: f.lessons[0].id, status: 'completed', progress: 100 });
    const before = plain(f.data);
    for (const path of [MIGRATION, PROGRESS_MIGRATION, WAIT_MIGRATION]) f.migration(path).up();
    assert.deepEqual(f.data, before, 'installing both contracts creates no enrollment or award');
    assert.equal(f.list().points, 0);
    const completed = f.finish();
    const saved = plain(f.data);
    for (const path of [MIGRATION, PROGRESS_MIGRATION, WAIT_MIGRATION]) f.migration(path).up();
    assert.deepEqual(f.data, saved);
    assert.equal(f.collections.tutorial_learning.fields.filter((field) => field.name === 'answer_retry_at').length, 1);
    for (const path of [WAIT_MIGRATION, PROGRESS_MIGRATION, MIGRATION]) f.migration(path).down();
    assert.throws(() => f.list(), /upgrade is not installed/);
    for (const key of ['createRule', 'updateRule', 'deleteRule']) assert.equal(f.collections.tutorial_progress[key], null);
    for (const path of [MIGRATION, PROGRESS_MIGRATION, WAIT_MIGRATION]) f.migration(path).up();
    assert.deepEqual(f.data, saved);
    assert.deepEqual(f.detail().enrollment.certificate, completed.enrollment.certificate);
});
...
test('learner detail and command responses are keyless while frozen snapshots and grading stay full', () => {
    ...
    assert.deepEqual(wrong.feedback, { correct: false,
        explanation: 'Not the expected answer. Review the lesson sections, then try again.', retry_after: 30 });
    f.expire();
...
test('routes require native users auth and keep responses uncached with bounded command bodies', () => {
    ...
    vm.runInNewContext(source('apps/pocketbase/pb_hooks/tutorial-learning.pb.js'), {
        ...
        require: () => ({ list: () => 'list', states: () => 'states', detail: () => 'detail', command: () => 'command', enrich: () => { enriched++; } }),
