...
// Created:     2026-09-23
// Depends:     apps/pocketbase/pb_hooks/classroom-media.js, apps/pocketbase/pb_hooks/classroom-realtime-lib.js
// EnumType:    Route
// EnumEdges:   CONSUMES apps/pocketbase/pb_hooks/classroom-media.js; CONSUMES apps/pocketbase/pb_hooks/classroom-realtime-lib.js
...
routerAdd('GET', '/api/classroom/health', (e) => {
    ...
    let installed = false;
    try { require(`${__hooks}/classroom-media.js`).schema(e.app); installed = true; } catch (_) { /* unavailable */ }
    const publishers = ($os.getenv('BUILDANDDO_CLASSROOM_PUBLISHERS') || '').split(',').filter((item) => item.trim());
    e.response.header().set('Cache-Control', 'no-store');
    return e.json(200, { ok: !config.reason && installed, route: 'classroom-realtime/v2',
