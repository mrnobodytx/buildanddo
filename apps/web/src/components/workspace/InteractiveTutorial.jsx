...
export default function InteractiveTutorial({ tutorialId, client, onSaved, onClose, opener }) {
    ...
    const load = useCallback(async () => {
        ...
        setLoading(true); setData(null); setFeedback(null); setAnswer(null); setChecks([]); setError('');
    ...
    const save = async (action, payload = {}, retry = false) => {
        ...
        setBusy(false);
        if (!result.ok) {
            if (result.reason !== 'uncertain') { setData(null); setFeedback(null); setAnswer(null); setChecks([]); }
            setError(result.error || 'Your learning access changed. Reopen the tutorial from your current account.');
            setUncertain(result.reason === 'uncertain'); return;
        }
            ...
            <div className="ph-no-capture min-w-0 space-y-6 break-words">
                ...
                {!loading && data && <>
                    ...
                    {step < 0 ? <div className="space-y-5"><p className="text-sm leading-7">{lesson.why}</p><ul className="list-disc space-y-2 pl-5 text-sm leading-6">{lesson.outcomes.map((outcome) => <li key={outcome}>{outcome}</li>)}</ul>
                        ...
                        <p className="text-sm leading-6 text-muted-foreground">Finish each section, self-report the practice checklist, and answer the final question to earn a completion certificate and 100 learning points. Answers are public in the lesson source and bundled previews; this is not a secure exam.</p>
