...
it('recovers a lost inferred response without another provider call', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Ask Buddi' })); await screen.findByRole('alert');
...
it('recovers the original session start even if the user edits the first message after response loss', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Ask Buddi' })); await screen.findByRole('region', { name: 'Proposed actions' });
...
it('demo mode makes no native or model requests', async () => {
    scope.demo = true; const user = userEvent.setup(); render(<Page />); await user.click(screen.getByRole('button', { name: 'Buddi', exact: true }));
    expect(screen.getByText(/Sign in to use saved Buddi sessions/)).toBeInTheDocument(); expect(pb.send).not.toHaveBeenCalled();
