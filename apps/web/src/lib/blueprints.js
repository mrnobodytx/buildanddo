 ...
 */
export function createBlueprintClient({ client, accountId, workspaceId, demo = false, isCurrent }) {
    ...
    return {
        async analyze(file, includePrompts = false) {
            ...
            try {
                ...
                const data = await client.send('/api/buildanddo/workspaces/' + encodeURIComponent(workspaceId) + '/blueprints', {
