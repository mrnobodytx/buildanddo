...
export function createAssistantClient({ client, workspaceId, accountId, isCurrent }) {
    ...
    const request = async (suffix, method, body, query) => {
        ...
        try {
            ...
            return current() ? { ok: false, error: error?.response?.message || "Could not confirm Buddi's response. Reload the session or retry the same message." } : { ok: false, stale: true };
        }
    };
    return {
