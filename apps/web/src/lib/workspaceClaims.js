...
export function createWorkspaceClaimClient({ client, collection, workspaceId, accountId, isCurrent,
    ...
    const send = async () => {
        ...
        if (busy) return failure('busy', 'Wait for the current save to finish.');
        if (!pending) return failure('invalid', 'There is no uncertain save to retry.');
        const lifetime = getLifetime();
        busy = true;
        pending.confirmed = false;
        try {
            const { body, operation, owner } = pending;
            const result = await observe(collection, operation, () => client.send(`/api/buildanddo/workspaces/${encodeURIComponent(workspaceId)}/claims`,
    ...
    return {
        async write(operation, recordId, data = {}, saved) {
            ...
            if (!CLAIM_COLLECTIONS.includes(collection) || !['create', 'update'].includes(operation) ||
                ...
                return failure('invalid', 'These reports are retained. Use a supported draft command or append a new report.');
            if (!data || typeof data !== 'object' || Array.isArray(data) || ['owner', 'workspace', 'claim_revision'].some((key) => Object.hasOwn(data, key)))
                return failure('invalid', 'Record authorship, scope and revisions are assigned by the native command.');
            let signature;
            try { signature = canonical([operation, recordId, data]); }
            catch { return failure('invalid', 'Use a bounded JSON request.'); }
            if (operation === 'update' && (!saved || !id(recordId) || saved.id !== recordId || saved.workspace !== workspaceId || !id(saved.owner)))
                return failure('invalid', 'Reload and select the saved record from this workspace before editing.');
            const revision = operation === 'create' ? 0 : saved.claim_revision ?? 0;
            if (!Number.isSafeInteger(revision) || revision < 0 || revision >= Number.MAX_SAFE_INTEGER)
                return failure('invalid', 'Reload a valid saved revision before editing.');
            let values;
            ...
            if (collection === 'daily_editions' && values.status === 'published') {
                if (operation !== 'update' || Object.keys(values).length !== 1)
                    return failure('invalid', 'Save the draft first; publish only its saved version.');
                ...
                if (values.status !== (collection === 'daily_editions' ? 'draft' : 'pending'))
                    return failure('invalid', 'This command cannot promote verification or provider health.');
            ...
            try {
                ...
                if (new TextEncoder().encode(JSON.stringify(body)).length > 30000)
                    return failure('invalid', 'Keep this request within 30000 UTF-8 bytes.');
                pending = { body, signature, operation, owner: operation === 'create' ? accountId : saved.owner };
            } catch { return failure('unavailable', 'A secure retry identifier is unavailable. Reload from the secure site before saving.'); }
            return send();
        },
        retry: send,
        confirm() {
