...
// Created:     2026-09-23
// Depends:     apps/web/src/lib/journey.js, apps/web/src/hooks/useWorkspaceRecords.js, apps/web/src/components/workspace/WorkspaceAssistant.jsx
// EnumType:    Widget
// EnumEdges:   CONSUMES apps/web/src/lib/journey.js; CONSUMES apps/web/src/hooks/useWorkspaceRecords.js; TRIGGERS apps/web/src/components/workspace/WorkspaceAssistant.jsx
// DAG Node:    none
...
export default function JourneyPage() {
    ...
    const [saved, setSaved] = useState(null); const [error, setError] = useState(''); const [saving, setSaving] = useState(false);
    ...
    const choose = (id) => {
        ...
        setSaved(null); setError(''); setStep(step + 1);
    };
    const restart = () => { setAnswers({}); setStep(0); setSaved(null); setError(''); };
    const save = async () => {
        if (pending.current || !compiled) return;
        pending.current = true; setSaving(true); setError('');
        try {
            const result = await create(compiled.mission);
            if (result.ok) setSaved(result.record || { id: '' });
            else if (!result.stale) setError(result.error || 'The draft could not be saved. Your answers are still here.');
    ...
    return <div className="space-y-6">
        ...
        {!done && <Card className="space-y-5 p-5">
            ...
            <div className="flex gap-3">{step > 0 && <Button variant="secondary" onClick={() => setStep(step - 1)}>Back</Button>}</div>
        </Card>}
        {compiled && <>
            ...
            <Card className="space-y-4 p-5">
                ...
                <div className="flex flex-wrap gap-3">
                    <Button onClick={save} disabled={demo || degraded || loading || saving || Boolean(saved)}>Save mission draft</Button>
