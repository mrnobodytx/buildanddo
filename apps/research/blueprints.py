...
# Created:     2026-09-17
# Depends:     apps/research/documents.py, apps/research/contracts.py
# EnumType:    Service
# EnumEdges:   EXTENDS apps/research/documents.py; DEPENDS_ON apps/research/contracts.py
# DAG Node:    none
# Intent:      Preserve document provenance while extracting bounded, untrusted blueprint requirements and topology for review.
# ───────────────────────────────────────────────────────────────

"""Structure document text using deterministic, text-only extraction rules."""
...
def _structure_document(data: bytes, name: str, value: str, truncated: bool) -> Blueprint:
    count = 0
    if name.lower().endswith('.pdf'):
        pypdf = importlib.import_module('pypdf')
        # Text and all admission checks come from documents.extract. This second
        # metadata read stays inside the same bounded child process.
...
def extract_blueprint(data: bytes, name: str) -> Blueprint:
    """Extend the existing document extractor for callers already in a parser sandbox."""
    value, _version, truncated = documents.extract(data, name)
    return _structure_document(data, name, value, truncated)
...
def parse_upload(data: bytes, name: str) -> dict[str, object]:
    ...
    try:
        blueprint = _structure_document(data, name, value, truncated)
